...
import Wrapper, { propTypes, defaultProps } from './Wrapper';
...
class Formsy extends React.Component {
  ...
  getChildContext = () => (
    {
      formsy: {
        ...
        isValidValue: (component, value) => this.runValidation(component, value)
          .then(info => info.isValid),
      },
    }
  )
  ...
  validate = (component) => {
    ...
    return this.runValidation(component)
      .then((validation) => {
        if (!validation) {
          return;
        }
  ...
  validateForm = () => {
    ...
    return Promise.all(this.inputs.map(async (component) => {
